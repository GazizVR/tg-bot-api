...
[file-tag: code-generated-file-fbb5f37c-b10c-4024-a125-01efa89d5ae6]
...
- **Long Polling Update Listener:** Concurrent update processing via Goroutines.
- **Message Operations:** Send, edit, delete messages, and upload documents/media.
- **Interactive Inline Keyboards:** Helpers to construct inline markups and handle callback queries.
- **Media & File Handling:** Direct file downloads by `file_id` and multipart document uploads.
- **Flexible Configuration:** Supports custom base API URLs (for self-hosted Telegram Bot API servers).
...
Below is a complete example showing how to initialize the client, listen for updates, send inline keyboards, and edit messages upon interaction.
...
| --- | --- |
| `NewClient(token, baseURL)` | Constructs a new Telegram API client instance. |
| `ListenUpdates(handler, allowed)` | Starts long polling and processes updates asynchronously in goroutines. |
| `SendMessage(chatId, text, markup, replyTo)` | Sends a text message with optional inline keyboard and reply parameters. |
| `DeleteMessage(chatId, messageId)` | Deletes a message from a chat. |
| `SendDocument(chatId, file, replyTo)` | Uploads and sends a document/file to a chat. |
| `EditMessageText(chatId, messageId, text, markup)` | Edits text and inline keyboard of an existing message. |
| `EditMessageReplyMarkup(chatId, messageId, markup)` | Updates inline buttons on an existing message. |
| `EditMessageMedia(chatId, messageId, mediaType, file, markup)` | Replaces existing message media with a new file. |
| `AnswerCallbackQuery(queryId)` | Acknowledges a callback query sent from an inline button. |
| `DownloadFile(fileId, dirPath)` | Retrieves file path metadata and downloads the file locally. |
...
├── LICENSE                 # MIT License
├── internal/
│   └── http.go             # Internal HTTP helpers (GET and multipart POST)
└── pkg/
    ├── client.go           # Client definition & update listening loop
    ├── dto.go              # API request/response DTOs & error parser
    ├── editMsgMethods.go   # Message editing methods
    ├── errors.go           # Package error definitions
    ├── fileMethods.go      # File retrieval & download functions
    ├── methods.go          # Core getUpdates & answerCallbackQuery methods
    ├── msgMethods.go       # Send/Delete message & document methods
    └── objects.go          # Core Telegram API types (Update, Message, InlineMarkup, etc.)
